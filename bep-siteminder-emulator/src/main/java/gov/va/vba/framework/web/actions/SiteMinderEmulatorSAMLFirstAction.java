 ...
 * 
 */
public class SiteMinderEmulatorSAMLFirstAction extends BaseAction  {

	private static Logger logger=Logger.getLogger(SiteMinderEmulatorSAMLFirstAction.class.getName());
	...
	public ActionForward executeAction(ActionMapping mapping,
		...
		logger.debug("SiteMinderEmulatorSAMLFirstAction::Module Prefix:- " + mapping.getModuleConfig().getPrefix());
		logger.debug("SiteMinderEmulatorSAMLFirstAction::Mapping path:- " + mapping.getPath());	
		...
		// The first request sent to SecurityFilter must have IDPSAML as the appid
		String targetURL = "http://localhost:7001/bep/security?appid="+appName+"&SMPORTALURL=http://localhost:7001/testeff&redirect=http://localhost:7001/siteminderemulator/siteMinderEmulatorSAMLFirst.jsp"; 
        
        List<NameValuePair> queryParams = new ArrayList<NameValuePair>(); 
        queryParams.add(new BasicNameValuePair("appid", appName)); // not sure why doing this
		...
		httpHeaderAttributes.put(BaseFilter.SM_APPLICATION_NAME, "IDPSAML"); // Do not use appName here. This in header means SAML request
