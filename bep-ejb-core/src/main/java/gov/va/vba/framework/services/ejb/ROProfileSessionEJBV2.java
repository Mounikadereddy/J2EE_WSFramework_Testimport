...
import gov.va.vba.framework.services.ROProfileSessionLocalV2;
import gov.va.vba.framework.services.ROProfileSessionRemoteV2;
import gov.va.vba.framework.services.ROProfileSessionV2;
...
@Stateless(mappedName="ROProfileSessionEJBV2")
@TransactionTimeoutSeconds(90)
@Local(ROProfileSessionLocalV2.class)
@Remote(ROProfileSessionRemoteV2.class)
public class ROProfileSessionEJBV2 implements ROProfileSessionV2 {
	...
	private static Logger logger=Logger.getLogger(ROProfileSessionEJBV2.class);
	 ...
	 * @param 
	 */
	public ROMappingProfileType getROCityCntyStateType(String pZip, AuditContext auditContext, Map extensions) throws ROMappingException{
		of = new ObjectFactory();
		roDataType = of.createROMappingProfileType();
		try{
			con = getROConnection(auditContext);
			...
			logger.error("auditContext="+auditContext, sqlex);
			...
			try {
				...
				logger.error("auditContext="+auditContext,se);
			}
		}	
		return roDataType;
	}
	 ...
	 * @param pZip Zip code input for query to the underlying database function.
	 */
	public String getROCityCntyStateXml(String pZip, AuditContext auditContext, Map extensions) throws ROMappingException {
		String roXmlString = null;
		ROMappingProfileType ro = getROCityCntyStateType(pZip, auditContext, extensions);
		try {
			roXmlString = convertTypeToXml(ro);
		} catch (JAXBException jxbe) {
			logger.error("auditContext="+auditContext, jxbe);
		} catch (IOException ioe) {
			logger.error("auditContext="+auditContext, ioe);
		}
		return roXmlString;
	}
	 ...
	 */
	public ROMappingProfileType getROType(String pZip, String pCountryName, String provinceName, String pLob, AuditContext auditContext, Map extensions) throws ROMappingException {
		of = new ObjectFactory();
		roDataType = of.createROMappingProfileType();
		try{
			con = getROConnection(auditContext);
	 ...
	 * @param pLob LOB input for query to the underlying database function.
	 */
	public String getROXml(String pZip, String pCountryName, String provinceName, String pLob, AuditContext auditContext, Map extensions) throws ROMappingException {
		String roXmlString = null;
		ROMappingProfileType ro = getROType(pZip, pCountryName, provinceName, pLob, auditContext, extensions);
		try {
			roXmlString = convertTypeToXml(ro);
		} catch (JAXBException jxbe) {
			logger.error("auditContext="+auditContext,jxbe);
	 ...
	 * @param pLob LOB input for query to the underlying database function.
	 */
	public ROMappingProfileType getROForeignCntriesType(String pLob, AuditContext auditContext, Map extensions) throws ROMappingException {
		of = new ObjectFactory();
		roDataType = of.createROMappingProfileType();
		try{
			con = getROConnection(auditContext);
	 ...
	 * @param pLob LOB input for query to the underlying database function.
	 */
	public String getROForeignCntriesXml(String pLob, AuditContext auditContext, Map extensions) throws ROMappingException {
		String roXmlString = null;
		ROMappingProfileType ro = getROForeignCntriesType(pLob, auditContext, extensions);
		try {
			roXmlString = convertTypeToXml(ro);
		} catch (JAXBException jxbe) {
			logger.error("auditContext="+auditContext,jxbe);
	 ...
	 */
	public ROMappingProfileType getROZipCodesType(String roNum, String lob, AuditContext auditContext, Map extensions) throws ROMappingException {
		of = new ObjectFactory();
		roDataType = of.createROMappingProfileType();
		try{
			con = getROConnection(auditContext);
	 ...
	 * @param lob LOB input for query to the underlying database function.
	 */
	public String getROZipCodesXml(String roNum, String lob, AuditContext auditContext, Map extensions) throws ROMappingException {
		String roXmlString = null;
		ROMappingProfileType ro = getROZipCodesType(roNum, lob, auditContext, extensions);
		try {
			roXmlString = convertTypeToXml(ro);
		} catch (JAXBException jxbe) {
			logger.error("auditContext="+auditContext,jxbe);
	 ...
	 * LOBs--CP, EDU, VRE.
	 */
	public ROMappingProfileType getRORegionStateType(String region, String lob, AuditContext auditContext, Map extensions) throws ROMappingException {
		of = new ObjectFactory();
		roDataType = of.createROMappingProfileType();
		try{
			con = getROConnection(auditContext);
	 ...
	 * @param lob LOB input for query to the underlying database function.
	 */
	public String getRORegionStateXml(String region, String lob, AuditContext auditContext, Map extensions) throws ROMappingException {
		String roXmlString = null;
		ROMappingProfileType ro = getRORegionStateType(region, lob, auditContext, extensions);
		try {
			roXmlString = convertTypeToXml(ro);
		} catch (JAXBException jxbe) {
			logger.error("auditContext="+auditContext,jxbe);
	 ...
	 */
	private Connection getROConnection(AuditContext auditContext) {
		try{
			InitialContext ctx = new InitialContext();
			DataSource ds = (DataSource)ctx.lookup(DS_NAME);
			con = ds.getConnection();	
		} catch(Exception excp) {
			logger.error("auditContext="+auditContext+", Error: getROCOnnection Failed",excp);
		}
		return con;
	}
