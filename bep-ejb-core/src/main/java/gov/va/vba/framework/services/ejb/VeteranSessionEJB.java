...
import gov.va.vba.framework.services.VeteranSession;
import gov.va.vba.framework.services.VeteranSessionLocal;
...
@Stateless(mappedName="VeteranSessionEJB")
@Local(VeteranSessionLocal.class)
@Remote(VeteranSessionRemote.class)
public class VeteranSessionEJB implements VeteranSession {

	private static Logger log = Logger.getLogger("vetsnet.services");
	private static Logger logger = Logger.getLogger(VeteranSessionEJB.class);
	
	@PersistenceContext( name="corpDbAdmin", unitName="corpDbAdmin")
	protected EntityManager em;
	
	public VeteranSessionEJB() {
	}
	 ...
	 */
	public Person findPerson(Long partId){
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "findPerson(partId)");
	...
	public Person findPerson(String ssn){
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "findPerson(ssn)");
		Person person = new Person();
		...
		if(null != ptcpnt.getPtcpntId()){
			ptcpnt = populatePtcpntSets(ptcpnt);
			person.setPtcpnt(ptcpnt);
		}
		return person;
	}
	
	public PtcpntAddr getMailingAddress(Set<PtcpntAddr> ptcpntAddrs) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "getMailingAddress");
		...
		if (tempAddr.size() > 0) {
			...
			for (PtcpntAddr ptAd : tempAddr) {
				if (null != testDate && null != ptAd.getEfctvDt()
	...
	public Person getSpouse(Person person) {
		...
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "getSpouse");
	...
	public boolean hasSpouse(Person person) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "hasSpouse");
		return (getSpouse(person) == null ? false : true);
	}
	 ...
	 * @return ArrayList
	 */
	public ArrayList<BnftClaim> populateBnftClaims(Person person) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "populateBnftClaims");
	 ...
	 * @return MltyPerson
	 */
	public MltyPerson populateMltyPerson(MltyPerson mltyPerson) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "populateMltyPerson");
	 ...
	 * @return Participant
	 */
	public Ptcpnt populatePtcpntSets(Ptcpnt ptcpnt) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "populatePtcpntSets");
...
//		}
	...
	public Set<IncomeDecn> getIncomeDecisions(Person person) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "getIncomeDecisions");
		Ptcpnt ptcpnt = person.getPtcpnt();
	 ...
	 * @return Person
	 */
	public Person populateSets(Person person) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "populateSets");
	 ...
	 * @return
	 */
	private XMLGregorianCalendar Date2XMLDate(Date date) {
	 ...
	 */
	protected ByteArrayOutputStream generateXML(VeteranDataType rootNode) {
		try {
	 ...
	 */
	public String getCorpXMLDataStream(String ssn) {
		new LegacyEJBAuditer().audit(VeteranSessionEJB.class.getName(), "getCorpXMLDataStream");
		log.info("Getting veteran information from corp.");

		try {
			Person person = findPerson(ssn);

			if (person != null) {
				VeteranDataType veteranNode = setRootNode(person);
				ByteArrayOutputStream os = generateXML(veteranNode);
	 ...
	 * @param rootNode
	 */
	private void setAddressNode(Person person, VeteranDataType rootNode) {
		...
		if (ptcpnt != null) {
			...
			PtcpntAddr mailingAddress = getMailingAddress(addresses);
	 ...
	 * @param rootNode
	 */
	private void setBirthInfoNode(Person person, VeteranDataType rootNode) {
		...
		if (null != person.getBrthdyDt()) {
			...
			birthInformationNode
					.setBirthDate(Date2XMLDate(person.getBrthdyDt()));
	 ...
	 * @return
	 */
	private ChildType setChildNode(PtcpntRlnshp ptcpntRlnshp) {
		...
		if (null != tempPerson.getBrthdyDt()) {
			childNode.setBirthDate(Date2XMLDate(tempPerson.getBrthdyDt()));
		}
	 ...
	 * @param rootNode
	 */
	private void setChildNodes(Person person, VeteranDataType rootNode) {
		...
		for (PtcpntRlnshp relationship : relationships) {
			if (relationship.getCompId().getPtcpntRlnshpTypeNm()
				...
				ChildType child = setChildNode(relationship);
	 ...
	 * @param rootNode
	 */
	private void setDeathInfoNode(Person person, VeteranDataType rootNode) {
		if (null != person.getDeathDt()) {
			DeathInformationType deathInformationNode = new DeathInformationType();
			deathInformationNode
					.setDeathDate(Date2XMLDate(person.getDeathDt()));
	 ...
	 * @param rootNode
	 */
	private void setIncomeInfoNode(Person person, VeteranDataType rootNode) {
		Set<IncomeDecn> decisions = getIncomeDecisions(person);
	 ...
	 * @param rootNode
	 */
	private void setMaritalStatusNode(Person person, VeteranDataType rootNode) {
		if (hasSpouse(person)) {
			...
			if (null != spouse) {
				...
				if (null != spouse.getBrthdyDt()) {
					spouseNode.setBirthDate(Date2XMLDate(spouse.getBrthdyDt()));
				}
	 ...
	 * @param rootNode
	 */
	private void setMilitaryServiceNode(Person person, VeteranDataType rootNode) {
	 ...
	 * @param rootNode
	 */
	private void setNursingHomeInfoNode(Person person, VeteranDataType rootNode) {
		...
		for (PtcpntInstzn institution : collection) {
			nursingHome.setDateEntered(Date2XMLDate(institution.getBeginDt()));
	...
	private VeteranDataType setRootNode(Person person) {
		VeteranDataType veteranNode = new VeteranDataType();

		if (null != person) {
			setIdentifiersNode(person, veteranNode);
			setAddressNode(person, veteranNode);

			setGenderNode(person, veteranNode);
			setMilitaryServiceNode(person, veteranNode);
			setBirthInfoNode(person, veteranNode);
			setNursingHomeInfoNode(person, veteranNode);
			setMaritalStatusNode(person, veteranNode);
			setDeathInfoNode(person, veteranNode);
			setIncomeInfoNode(person, veteranNode);
			setChildNodes(person, veteranNode);
		}
		...
		if (null != person.getRatingDecns()
			...
			for (RatingDecn ratingDecn : person.getRatingDecns()) {
				if (null != ratingDecn.getDsbltyEvaltn()) {
					setDisabilityInfoNode(ratingDecn, veteranNode);
				}
			}
		}
