...
import gov.va.vba.framework.services.VadirSessionLocalV2;
import gov.va.vba.framework.services.VadirSessionRemoteV2;
import gov.va.vba.framework.services.VadirSessionV2;
...
@Stateless(mappedName="VadirSessionEJBV2")
@Local(VadirSessionLocalV2.class)
@Remote(VadirSessionRemoteV2.class)
public class VadirSessionEJBV2 implements VadirSessionV2 {

	private static Logger logger=Logger.getLogger(VadirSessionEJBV2.class);
	@Override
	public String getVadirData(String vonAppUser, String vonAppTerm, String vaIdIn, AuditContext auditContext, Map extensions){
		...
		con = setVadirConnection(auditContext);
		...
		try{
			...
			logger.error("auditContext="+auditContext,jbex);
		}
	 ...
	 */
	private Connection setVadirConnection(AuditContext auditContext) {
...
//		    ctx = new InitialContext(ht);
		...
		{
			logger.error("auditContext="+auditContext+", Failed",excp);
		}
