...
import gov.va.vba.framework.services.EmailServiceRemoteV2;
import gov.va.vba.framework.services.EmailServiceV2;
import gov.va.vba.framework.services.ejb.ContentType; // for legacy backwards compatibility use ContentType in folder /ejb
...
@Stateless(name = "VbaEmailServiceV2", mappedName = "VbaEmailServiceV2")
@Local(EmailServiceV2.class)
@Remote(EmailServiceRemoteV2.class)
public class EmailServiceImplV2 implements EmailServiceV2 {
  private static Logger logger = Logger.getLogger(EmailServiceImplV2.class.getName());
...
private Hashtable<String, String> ldapConnectionProperties;
  ...
  @Override
  public void sendEmail(String fromName, String from, String to, String cc, String bcc, String subject, String body, ContentType contentType, Collection<File> attachments, boolean encrypt, AuditContext auditContext, Map extensions) throws MessagingException, IOException {
		...
		if(encrypt){
			...
			bodyPart = encrypt(bodyPart, auditContext);
  ...
  private MimeBodyPart encrypt(MimeBodyPart body, AuditContext auditContext) {
		SMIMEEnvelopedGenerator generator = new SMIMEEnvelopedGenerator();
		for(X509Certificate cert: getCertificates(auditContext))
  ...
  private List<X509Certificate> getCertificates(AuditContext auditContext) {
		List<X509Certificate> certs = new ArrayList<X509Certificate>();
		for(String address: tos){
			X509Certificate cert = getCertificate(address, auditContext);
  ...
  private X509Certificate getCertificate(String email, AuditContext auditContext) {
		...
		try {
			...
			while (results.hasMore()) {
				...
				try {
					...
					logger.error("auditContext="+auditContext,e); //Certificate is not of X.509 type
				}
			}
		} catch (NamingException e) {
			logger.error("auditContext="+auditContext,e);
		} finally {
			if (results != null) try {results.close();} catch (Exception e) { logger.error("Ignore any exceptions from closing results: "+ e);}
		}
